package com.mindscriptact.mvcExpressLogger
{
    ...
    import com.mindscriptact.mvcExpressLogger.screens.MvcExpressTimelineScreen;
    ...
    public class MvcExpressLogger
    {
        ...
        static public const TIMELINE_TAB:String = "TIMELINE_TAB";
        ...
        public static var frameCounter:uint = 0;
        public static var frameDictionary:Vector.<Vector.<LogEntryVO>>;
        ...
        static public function init(stage:Stage, x:int = 0, y:int = 0, width:int = 900, height:int = 400, alpha:Number = 0.9, autoShow:Boolean = false, initTab:String = "LOG", openKeyCode:int = 192, isCtrlKeyNeeded:Boolean = true, isShiftKeyNeeded:Boolean = false, isAltKeyNeeded:Boolean = false):void
        {
            ...
            if (stage)
            {
                ...
                {
                    MvcExpress;
                    mvcExpressClass = MvcExpress
                    ModuleManager;
                    moduleManagerClass = ModuleManager;
                }
                catch (error:Error)
                {
                }
        ...
        private function initLog():void{
            if (!frameDictionary)
            {
                frameDictionary = new Vector.<Vector.<LogEntryVO>>()
            }
            while(frameDictionary.length<=frameCounter){
                frameDictionary.push( new Vector.<LogEntryVO>());
            }
        }
        ...
        private function debugMvcExpress(traceObj:Object):void
        {
            ...
            if (traceObj.canPrint)
            {
                ...
                logText += traceObj + "\n";
                //
                var logVO:LogEntryVO= new LogEntryVO();
                logVO.timer=getTimer();
                logVO.frameNumber=frameCounter;
                logVO.logMSG=traceObj+" ";
                frameDictionary[frameCounter].push(logVO);
                if (isLogShown)
                {
        ...
        private function showLogger():void
        {
            ...
            if (!logWindow)
            {
                ...
                var timelineButton:Mvce_PushButton = new Mvce_PushButton(logWindow, 0, -0, TIMELINE_TAB, handleButtonClick);
                timelineButton.toggle = true;
                timelineButton.width = 120;
                timelineButton.x = 670;
                allButtons.push(timelineButton);
            ...
            if (currentTogleButton != targetButton)
            {
                ...
                switch (currentTabButtonName)
                {
                    case VISUALIZER_TAB:
                        ...
                        break;
                    case TIMELINE_TAB:
                        currentScreen = new MvcExpressTimelineScreen(width - 6, height - 52);
                        currentScreen.x = 3;
                        currentScreen.y = 25;
                        visualizerManager.manageThisScreen(currentModuleName, currentScreen as MvcExpressTimelineScreen);
                        break;
                    default:
